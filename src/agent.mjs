...
function queueSwap(params) {
  const result = _txQueue.then(() =>
    ACP_ENABLED ? callToolViaACP(params) : callTool('execute_swap', params)
  )
...
// Initialized with basic defaults, then overwritten by loadTokenConfig().
let STABLECOINS = new Set(['USDC', 'USDT', 'DAI', 'USDBC'])
...
async function loadTokenConfig() {
  try {
    ...
    STABLECOINS = new Set(TOKEN_CONFIG.filter(t => t.is_stablecoin).map(t => t.symbol))
...
function parseDCA(text) {
  ...
  if (mSingle) {
    ...
    const isStable = STABLECOINS.has(tok)
    const tokenIn  = fromMatch ? fromMatch[1].toUpperCase() : (isStable ? 'WETH' : 'USDC')
    const tokenOut = isStable ? 'USDC' : tok
...
function buildPlanSystemPrompt() {
  const stables = [...STABLECOINS].join(', ')
  const nonStables = SWAPPABLE_SYMBOLS.filter(s => !STABLECOINS.has(s)).join(', ')
...
- "buy TOKEN" (no source specified) = token_in defaults to first stablecoin, token_out:TOKEN
- "sell TOKEN" (no destination specified) = token_in:TOKEN, token_out defaults to first stablecoin
...
export async function startAgent(log) {
  ...
  if (tokenData) {
    log(`tokens loaded: ${SWAPPABLE_SYMBOLS.length} symbols, ${STABLECOINS.size} stablecoins`)
  } else {
