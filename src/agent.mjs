...
function parseMomentumDCA(text) {
  ...
  const tokenIn  = (p.token_in ?? (STABLECOINS.has(tokenOut) ? 'WETH' : 'USDC')).toUpperCase()
  const signalToken = STABLECOINS.has(tokenOut) ? tokenIn : tokenOut
  return {
    ...
    selling:    STABLECOINS.has(tokenOut),
  }
}
